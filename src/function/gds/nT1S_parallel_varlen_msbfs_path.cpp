...
void nT1SParallelVarlenMSBFSPath::exec() {
    ...
    if (laneWidth == 8) {
        auto ifeMorsel = std::make_unique<MSBFSVarlenPathIFEMorsel<uint8_t>>(extraData->upperBound,
            1, numNodes - 1);
        auto count = 0;
        ...
        auto ifeMorsel = std::make_unique<MSBFSVarlenPathIFEMorsel<uint16_t>>(extraData->upperBound,
            1, numNodes - 1);
        auto count = 0;
        for (auto offset = 0u; offset < numNodes; offset++) {
            if (!inputMask->isMasked(offset)) {
        ...
        auto ifeMorsel = std::make_unique<MSBFSVarlenPathIFEMorsel<uint32_t>>(extraData->upperBound,
            1, numNodes - 1);
        auto count = 0;
        for (auto offset = 0u; offset < numNodes; offset++) {
            if (!inputMask->isMasked(offset)) {
        ...
        auto ifeMorsel = std::make_unique<MSBFSVarlenPathIFEMorsel<uint64_t>>(extraData->upperBound,
            1, numNodes - 1);
        auto count = 0;
        for (auto offset = 0u; offset < numNodes; offset++) {
            if (!inputMask->isMasked(offset)) {
