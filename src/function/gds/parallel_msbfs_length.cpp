                ...
                for (auto offset = frontierMorsel.startOffset; offset < frontierMorsel.endOffset; offset++) {
                    if (msbfsIFEMorsel->current[offset]) {
                        ...
                        for (auto nbrIdx = csrEntry->csr_v[posInCSR]; nbrIdx < csrEntry->csr_v[posInCSR + 1]; nbrIdx++) {
                            ...
                            auto shouldBeActive = msbfsIFEMorsel->current[offset] & ~msbfsIFEMorsel->seen[nbrOffset];
                            if (shouldBeActive) {
                                auto oldVal = msbfsIFEMorsel->next[nbrOffset];
                                auto newVal = oldVal | shouldBeActive;
                                ...
                                while (shouldBeActive) {
                                    if (shouldBeActive & 0x1) {
                                        auto exact1BytePos = nbrOffset * 8 + (8 - pos);
                                        newVal = msbfsIFEMorsel->currentLevel + 1;
                                        __atomic_store_n(&msbfsIFEMorsel->pathLength[exact1BytePos], newVal, __ATOMIC_RELAXED);
                    ...
                    if (msbfsIFEMorsel->current[offset]) {
                        ...
                        do {
                            ...
                            for (auto j = 0u; j < size; j++) {
                                ...
                                auto shouldBeActive = msbfsIFEMorsel->current[offset] & ~msbfsIFEMorsel->seen[dstNodeID.offset];
                                if (shouldBeActive) {
                                    auto oldVal = msbfsIFEMorsel->next[dstNodeID.offset];
                                    auto newVal = oldVal | shouldBeActive;
                                    while (__sync_bool_compare_and_swap_1(
                                    ...
                                    while (shouldBeActive) {
                                        if (shouldBeActive & 0x1) {
                                            auto exact1BytePos = dstNodeID.offset * 8 + (8 - pos);
                                            newVal = msbfsIFEMorsel->currentLevel + 1;
                                            __atomic_store_n(&msbfsIFEMorsel->pathLength[exact1BytePos], newVal, __ATOMIC_RELAXED);
                                            shouldBeActive = shouldBeActive >> 1;
                                            pos++;
                    ...
                    if (msbfsIFEMorsel->current[offset]) {
                        ...
                        for (auto nbrIdx = csrEntry->csr_v[posInCSR]; nbrIdx < csrEntry->csr_v[posInCSR + 1]; nbrIdx++) {
                            ...
                            if (shouldBeActive) {
                                ...
                                while (shouldBeActive) {
                                    if (shouldBeActive & 0x1) {
                                        auto exact1BytePos = nbrOffset * 16 + (16 - pos);
                                        newVal = msbfsIFEMorsel->currentLevel + 1;
                                        __atomic_store_n(&msbfsIFEMorsel->pathLength[exact1BytePos], newVal, __ATOMIC_RELAXED);
                                        shouldBeActive = shouldBeActive >> 1;
                                        pos++;
                        ...
                        do {
                            ...
                            for (auto j = 0u; j < size; j++) {
                                ...
                                if (shouldBeActive) {
                                    ...
                                    while (shouldBeActive) {
                                        if (shouldBeActive & 0x1) {
                                            auto exact1BytePos = dstNodeID.offset * 16 + (16 - pos);
                                            newVal = msbfsIFEMorsel->currentLevel + 1;
                                            __atomic_store_n(&msbfsIFEMorsel->pathLength[exact1BytePos], newVal, __ATOMIC_RELAXED);
                                            shouldBeActive = shouldBeActive >> 1;
                                            pos++;
                    ...
                    if (msbfsIFEMorsel->current[offset]) {
                        ...
                        for (auto nbrIdx = csrEntry->csr_v[posInCSR]; nbrIdx < csrEntry->csr_v[posInCSR + 1]; nbrIdx++) {
                            ...
                            if (shouldBeActive) {
                                ...
                                while (shouldBeActive) {
                                    if (shouldBeActive & 0x1) {
                                        auto exact1BytePos = nbrOffset * 32 + (32 - pos);
                                        newVal = msbfsIFEMorsel->currentLevel + 1;
                                        __atomic_store_n(&msbfsIFEMorsel->pathLength[exact1BytePos], newVal, __ATOMIC_RELAXED);
                                        shouldBeActive = shouldBeActive >> 1;
                                        pos++;
                        ...
                        do {
                            ...
                            for (auto j = 0u; j < size; j++) {
                                ...
                                if (shouldBeActive) {
                                    ...
                                    while (shouldBeActive) {
                                        if (shouldBeActive & 0x1) {
                                            auto exact1BytePos = dstNodeID.offset * 32 + (32 - pos);
                                            newVal = msbfsIFEMorsel->currentLevel + 1;
                                            __atomic_store_n(&msbfsIFEMorsel->pathLength[exact1BytePos], newVal, __ATOMIC_RELAXED);
                                            shouldBeActive = shouldBeActive >> 1;
                                            pos++;
                    ...
                    if (msbfsIFEMorsel->current[offset]) {
                        ...
                        for (auto nbrIdx = csrEntry->csr_v[posInCSR]; nbrIdx < csrEntry->csr_v[posInCSR + 1]; nbrIdx++) {
                            ...
                            if (shouldBeActive) {
                                ...
                                while (shouldBeActive) {
                                    if (shouldBeActive & 0x1) {
                                        auto exact1BytePos = nbrOffset * 64 + (64 - pos);
                                        newVal = msbfsIFEMorsel->currentLevel + 1;
                                        __atomic_store_n(&msbfsIFEMorsel->pathLength[exact1BytePos], newVal, __ATOMIC_RELAXED);
                                        shouldBeActive = shouldBeActive >> 1;
                                        pos++;
    ...
    void executenT1SPolicy(processor::ExecutionContext *executionContext) {
        ...
        if (laneWidth == 8) {
            auto ifeMorsel = std::make_unique<MSBFSIFEMorsel<uint8_t>>(extraData->upperBound,
                1, numNodes - 1, std::vector<common::offset_t>());
            auto count = 0;
        ...
        auto ifeMorsel = std::make_unique<MSBFSIFEMorsel>(extraData->upperBound, 1, numNodes - 1,
