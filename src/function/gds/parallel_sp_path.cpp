...
public:
    ...
    void exec(processor::ExecutionContext* executionContext) override {
        ...
        if (bfsPolicy == "nT1S") {
            auto execnT1SPolicy = std::make_unique<nT1SParallelSPPath>(executionContext,
                sharedState, bindData.get(), parallelUtils.get());
            execnT1SPolicy->exec();
        } else if (bfsPolicy == "1T1S") {
            auto exec1T1SPolicy = std::make_unique<_1T1SParallelSPPath>(executionContext,
                sharedState, bindData.get(), parallelUtils.get());
            exec1T1SPolicy->exec();
        } else if (bfsPolicy == "nTkS") {
            auto execnTkSPolicy = std::make_unique<nTkSParallelSPPath>(executionContext,
                sharedState, bindData.get(), parallelUtils.get());
            execnTkSPolicy->exec();
        } else {
