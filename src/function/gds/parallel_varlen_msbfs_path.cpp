...
#include "function/gds/1T1S_parallel_varlen_msbfs_path.h"
#include "function/gds/gds_function_collection.h"
#include "function/gds/nT1S_parallel_varlen_msbfs_path.h"
...
class ParallelMSBFSVarLenPaths : public GDSAlgorithm {
public:
    ParallelMSBFSVarLenPaths() = default;
    ParallelMSBFSVarLenPaths(const ParallelMSBFSVarLenPaths& other) = default;
    ...
    bool hasPathOutput() const override { return true; }
    ...
    void exec(processor::ExecutionContext* executionContext) override {
        ...
        if (bfsPolicy == "nT1S") {
            auto executenT1SPolicy = std::make_unique<nT1SParallelVarlenMSBFSPath>(executionContext,
                sharedState, bindData.get(), parallelUtils.get());
            executenT1SPolicy->exec();
        } else if (bfsPolicy == "1T1S") {
            auto execute1T1SPolicy = std::make_unique<_1T1SParallelVarlenMSBFSPath>(
                executionContext, sharedState, bindData.get(), parallelUtils.get());
            execute1T1SPolicy->exec();
        } else if (bfsPolicy == "nTkS") {
            auto executenTkSPolicy = std::make_unique<nTkSParallelVarlenMSBFSPath>(executionContext,
                sharedState, bindData.get(), parallelUtils.get());
    ...
    std::unique_ptr<GDSAlgorithm> copy() const override {
        return std::make_unique<ParallelMSBFSVarLenPaths>(*this);
    }
};

function_set ParallelMSBFSVarLenPathsFunction::getFunctionSet() {
    function_set result;
    auto function =
        std::make_unique<GDSFunction>(name, std::make_unique<ParallelMSBFSVarLenPaths>());
