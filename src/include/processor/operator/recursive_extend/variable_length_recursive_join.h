...
public:
    VariableLengthRecursiveJoin(uint8_t lowerBound, uint8_t upperBound,
        ...
        const DataPos& tmpDstNodeIDVectorPos, std::shared_ptr<MorselDispatcher> morselDispatcher,
        ...
        : BaseRecursiveJoin{lowerBound, upperBound, nodeTable, std::move(sharedState),
              ...
              dstNodeIDVectorPos, tmpDstNodeIDVectorPos,
std::move(morselDispatcher),std::move(child), id, paramsString, std::move(recursiveRoot)} {}
   ...
   VariableLengthRecursiveJoin(uint8_t lowerBound, uint8_t upperBound,
        ...
        : BaseRecursiveJoin{lowerBound, upperBound, nodeTable, std::move(sharedState),
              std::move(vectorsToScanPos), std::move(colIndicesToScan), srcNodeIDVectorPos,
              dstNodeIDVectorPos, tmpDstNodeIDVectorPos, std::move(morselDispatcher), id,
paramsString, std::move(recursiveRoot)} {}
    ...
    inline std::unique_ptr<PhysicalOperator> clone() override {
        return std::make_unique<VariableLengthRecursiveJoin>(lowerBound, upperBound, nodeTable,
            ...
            tmpDstNodeIDVectorPos, morselDispatcher, id, paramsString, recursiveRoot->clone());
    }
