...
public:
    ScanRelTableLists(DirectedRelTableData* tableData, vector<uint32_t> propertyIds,
        ...
        : ScanRelTable{inNodeIDVectorPos, std::move(outputVectorsPos),
          ...
          tableData{tableData} {
        scanState = make_unique<RelTableScanState>(std::move(propertyIds), RelTableDataType::LISTS);
    }
