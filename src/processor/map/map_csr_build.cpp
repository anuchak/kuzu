...
std::unique_ptr<PhysicalOperator> PlanMapper::mapCSRBuild(
    ...
    for (auto& property : logicalCSRBuild->getRel()->getPropertyExpressions()) {
        auto expression = (binder::PropertyExpression*)property.get();
        if (expression->getPropertyName().find("weight") != std::string::npos) {
            relWeightPropertyVectorPos = DataPos(schema->getExpressionPos(*property));
        }
    }
    ...
    return std::make_unique<CSRIndexBuild>(std::move(resultSetDescriptor), commonNodeTableID,
        ...
        relWeightPropertyVectorPos, sharedState, std::move(child), getOperatorID(),
        logicalCSRBuild->getExpressionsForPrinting());
}
