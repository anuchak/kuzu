...
void CSRIndexBuild::initLocalStateInternal(
    ...
    relWeightPropertyVector = resultSet->getValueVector(relWeightPropertyVectorPos).get();
}
...
void CSRIndexBuild::executeInternal(kuzu::processor::ExecutionContext* context) {
    ...
    while (children[0]->getNextTuple(context)) {
        ...
        if (!csr_v[boundNode.offset]) {
            ...
            totalSizeAllocated +=
                (sizeof(csrEntry) + 3 * totalNbrOffsets * sizeof(common::offset_t));
        } else {
        ...
        for (auto i = 0u; i < totalNbrOffsets; i++) {
            ...
            auto relWeightProperty = relWeightPropertyVector->getValue<int64_t>(pos);
            csr_v[boundNode.offset]->nbrNodeOffsets[i] = nbrNode.offset;
            csr_v[boundNode.offset]->relIDOffsets[i] = relID.offset;
            csr_v[boundNode.offset]->relWeightProperties[i] = relWeightProperty;
        }
    }
