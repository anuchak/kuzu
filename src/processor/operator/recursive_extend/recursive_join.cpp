...
void RecursiveJoin::computeMSBFSMorsel(kuzu::processor::ExecutionContext* context) {
    auto msBFSMorsel = (reinterpret_cast<MSBFSMorsel<false>*>(bfsMorsel.get()));
    uint32_t *temp, *x = msBFSMorsel->visit, *next_ = msBFSMorsel->next;
...
bool RecursiveJoin::doMSBFS(uint32_t * seen, uint32_t * curFrontier, uint32_t * nextFrontier,
...
void RecursiveJoin::callMSBFSRecursivePlan(const uint32_t * seen, const uint32_t * curFrontier,
