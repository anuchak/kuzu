...
void DirectedRelTableData::initializeColumns(
    ...
    adjColumn = make_unique<AdjColumn>(StorageUtils::getAdjColumnStructureIDAndFName(
...
void DirectedRelTableData::initializeLists(
    RelTableSchema* tableSchema, BufferManager& bufferManager, WAL* wal) {
    adjLists = make_unique<AdjLists>(StorageUtils::getAdjListsStructureIDAndFName(
                                         wal->getDirectory(), tableSchema->tableID, direction),
...
RelTable::RelTable(const Catalog& catalog, table_id_t tableID, BufferManager& bufferManager,
    ...
    fwdRelTableData = make_unique<DirectedRelTableData>(tableID, tableSchema->getBoundTableID(FWD),
        FWD, listsUpdatesStore.get(), bufferManager,
        tableSchema->isSingleMultiplicityInDirection(FWD));
    bwdRelTableData = make_unique<DirectedRelTableData>(tableID, tableSchema->getBoundTableID(BWD),
        BWD, listsUpdatesStore.get(), bufferManager,
...
vector<AdjLists*> RelTable::getAllAdjLists(table_id_t boundTableID) {
...
vector<AdjColumn*> RelTable::getAllAdjColumns(table_id_t boundTableID) {
